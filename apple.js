...
riot.tag2('gc-inputlist', '<div if="{multiselect}" class="tags" name="region"> </div> <gc-input label="{opts.label}" placeholder="{opts.placeholder}" name="input" on-valuechange="{updateList}"></gc-input> <ul id="search-results" class="list hidden"></ul>', '', '', function(opts) {
'use strict';
...
riot.tag2('pm-filter', '<div id="filter"> <div class="item"> <gc-input name="startdate" pikaday="true" value="{new Date()}" on-focuschange="{dateChanged}"></gc-input> <gc-input name="enddate" pikaday="true" value="{new Date()}" disabled="true"></gc-input> </div> <div class="item"> <gc-inputlist name="city" type="text" list="{window.pricing_matrix_cities}" key="title" placeholder="City"></gc-inputlist> <gc-inputlist name="type" type="text" list="{window.pricing_matrix_cities}" key="title" placeholder="Type"></gc-inputlist> </div> </div> <div> <gc-button class="item search-button" name="button" type="submit" text="Search" on-buttonclick="{gimmeGimme}"></gc-button> </div>', '', '', function(opts) {
"use strict";

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

// Even though this is a global settings variable we wrap it in a promise interface to abstract it away in case we need to change to using
// ajax calls later on.
...
var CityService = function () {
  function CityService() {
    ...
    this.cities = window.pricing_matrix_cities;
  }

  CityService.prototype.index = function index() {
    var _this = this;
  ...
  CityService.prototype.retrieve = function retrieve(id) {
    var _this2 = this;

    return new Promise(function (resolve, reject) {
      results = _this2.cities.filter(function (city) {
        return city.id == id;
      });
      resolve(results[0]);
    });
...
//let CityService = require('CityService');
var cities = new CityService();
console.log(cities);
this.dateChanged = function (value, sender) {
	if (sender.name === "startdate") {
		_this.tags.enddate.update({
			value: sender.value
		});
	}
};

this.gimmeGimme = function (e) {
	console.log(e);
};
});

riot.tag2('pm-header', '<gc-nav name="navbar"> <li><gc-badge base="Revew Your Quote" badge="42"></gc-badge></li> <li><a href="/user/logout">Logout</a></li> </gc-nav>', '', '', function(opts) {
'use strict';
...
this.on('mount', function () {
	if (opts.isadmin) {
		_this.navbar.firstChild.style.marginTop = '30px';
	}
});
});
riot.tag2('pm-search', '<pm-header isadmin="{this.isAdmin}"></pm-header> <pm-filter></pm-filter>', '', '', function(opts) {
'use strict';

var _this = this;

this.on('before-mount', function () {
	// Get rid of drupal breadcrumb taking up space
	document.getElementById('branding').style.display = 'none';
	// May not need this on live
	if (document.getElementById('console')) {
		document.getElementById('console').style.display = 'none';
	}
	// Check to see if current user is an admin
	var rolesObj = window.currentUser.roles;
	_this.isAdmin = Object.keys(rolesObj).map(function (key) {
		return rolesObj[key];
	}).indexOf('administrator') >= 0;
	_this.update();
});
});
